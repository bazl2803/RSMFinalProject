namespace Application.Sales.GetSalesSummary
{
    ...
    public sealed class GetSalesSummaryQueryHandler
        : IQueryHandler<GetSalesSummaryQuery, List<SalesSummaryResponse>>
    {
        ...
        public async Task<Result<List<SalesSummaryResponse>>> Handle(
            GetSalesSummaryQuery request,
            CancellationToken cancellationToken)
        {
            if (request.PageSize <= 0)
                return Result.Failure<List<SalesSummaryResponse>>(
                    new Error("GetSalesSummary", "Specify page size"));
